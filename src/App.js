...
function App() {
  ...
  const sendDonation = async () => {
    try {
      ...
      const buyMeCoffeeContract = new ethers.Contract(
        CONTRACT_ADDRESS,
        contractABI,
        signer
      )

      let depositTxn = await buyMeCoffeeContract.deposit(
        `${donationValue}`,
        `${message}`,
      ...
      return (
        ...
        >
          <a href='https://metamask.io/' target='_blank'>
            <Button width='full'>Download Metamask</Button>
          </a>
        </Box>
      )
    }
    ...
    if (wallet.status === 'connected') {
      return (
        <VStack spacing={6} py={5}>
          {wallet.account === OWNER_ADDRESS ? null : (
            <>
          ...
          {isLoading ? (
            ...
            <Button
              ...
              colorScheme='blue'
              onClick={() => {
                wallet.account === OWNER_ADDRESS ? withdraw() : sendDonation()
              }}
            >
              {wallet.account === OWNER_ADDRESS
                ? `Withdraw`
                : `Send Ξ ${donationValue}`}
            </Button>
          )}
        </VStack>
      )
      ...
      >
        <Box mx={10}>
          ...
          <Box py={3}>
            ...
            {wallet.account === OWNER_ADDRESS ? (
              <Text fontSize='md'>Withdraw your donations below!</Text>
            ) : (
              <Text fontSize='md'>
                If you would like to support {YOUR_NAME} in his <br />
                creative endeavors feel free to use this dApp <br /> to buy him
                a coffee with crypto! <br />{' '}
              </Text>
            )}{' '}
          </Box>{' '}
          {renderContent()}
        ...
        <Box pos='absolute' bottom='2%'>
          ...
          >
            <a target='_blank' href={GITHUB_LINK}>
              <IconButton aria-label='github social' icon={<FaGithub />} />
            </a>
            <a target='_blank' href={TWITTER_LINK}>
              <IconButton aria-label='Twitter social' icon={<FaTwitter />} />
            </a>
          </ButtonGroup>
        </Box>
      </Flex>
